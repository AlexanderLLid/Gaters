...
container = hou.node("/obj").createNode("geo", "BODY_PLAN")
for child in container.children():
    child.destroy()
stash = container.createNode("stash", "BODY_PLAN_MESH")
...
for key in ("placements", "connections", "cells", "mirrors"):
    stash.setUserData(key, json.dumps(body[key], sort_keys=True))
output = container.createNode("null", "OUT_BODY_PLAN")
output.setInput(0, stash)
output.setDisplayFlag(True)
output.setRenderFlag(True)
container.layoutChildren()
scene_path = output_dir / "stick-humanoid.hipnc"
hou.hipFile.save(str(scene_path))
print(f"HOUDINI_BODY_PLAN_BUILD_PASS scene={scene_path}")
