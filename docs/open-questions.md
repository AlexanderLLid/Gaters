...
— no separate frontier phenomenon, the contest on contested frontier worlds ([[Gates|Gates]] FRONTIER-1); **no sealed/fail-closed Gate state and no re-key** — home protection is being hard to find (a decaying share-trail) + relocation + bounded loss ([[gates|Gates]] GATE-2, [[coordinates|Coordinates & Obscurity]]).

> **Canon note.** The home is **mortal**, protected by **obscurity** (a decaying avatar-share
> trail), not a wall; only your _existence_ is unloseable. There is **no sealed Gate state** and **no
> re-key** — a found home is dialable online or off, and the trail goes cold on its own as shares age.
> See [[coordinates|Coordinates & Obscurity]] and [[consciousness-link|Consciousness Link]].
...
7. **What reaches an offline / unfound base** — confirm the split (info/comms yes; goods/energy/people no, only through a dialed throat). Open sub-part: does an offline owner get a **raid notification**, and can they recall to defend? A free "you're being raided" alert + instant recall would revive the babysitting tax — bears on [[consciousness-link|Consciousness Link]] and [[coordinates|the odds]].
...
16. ~~**Homesteader progression ceiling**~~ — resolved (PROG-2): a peaceful homesteader progresses _forever_ without opening out, up to a **soft ceiling** (one `home-reachable` boolean per tech node; frontier tiers flip to `false`), then plateaus — never decays, never punished. Stagnation as a ceiling, not a leak. The homesteader's **safety** is **being unfound** — a pure-PvE never-leaver sheds no avatar-shares, so stays unfound; the plateau still holds. ([[progression|Progression]], [[coordinates|Coordinates & Obscurity]])
...
18. ~~**Can coordinates go stale?**~~ — resolved: yes, **passively**. Avatar-shares **decay over time** (no explicit re-key action — that was removed); a trail assembled from old shares loses the lock as they age out, and relocating claims a fresh coordinate. Share-decay rate + threshold _k_ are the master knobs for the online:offline raid ratio. ([[coordinates|Coordinates & Obscurity]])
