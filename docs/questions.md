...
- **#5 Avatar origin** — what the avatar physically is, where the body comes from, how the mind connects. Full options report + recommendation in `raw/avatar-origin-options.md` (4 archetypes: printed/cloned, seeded-evolved organism, mask-kernel, creature/symbiote; 6 invariants treated as settled). The **body-flavor pick is not made** — until it is, every doc uses only the origin-agnostic baseline (disposable avatar, respawn at an anchor, mask, coordinate-shares, two-layer XP). Also couples: whether the gate-read **pattern** (GATE-4) follows the pilot across bodies or is per-avatar. Touches [[systems#Presence, respawn & XP|Presence & Respawn]], [[systems#Mask energy|Mask Energy]], [[systems#Coordinates & obscurity|Coordinates]], [[world#United Gate Coalition|Coalition]].
...
- **#8 Raider anonymity — what peers do with patterns.** Mechanism decided (GATE-4): gates read the pattern of whatever crosses; the Coalition maps patterns to charters (it always knows you); peers get recognition — "the same being as yesterday" — never names. Open: what peers can _do_ with a recognized pattern — tracking tools, bounty posting, how long recognition persists. ([[world#United Gate Coalition|Coalition]], [[systems#Gates|Gates]], [[systems#Raiding|Raiding]])
...
- **#10 Frontier-world specifics** — rank scale (E–S vs. 1–10), frontier-loot vs. raid-loot balance (including the dead-house discount: PvE base loot < player-base loot at tier, HOUSE-2), gates-per-world distribution (FRONTIER-3), the self-misdial rate/trigger (FRONTIER-2), and the exit-dial economics (core cost vs. loot value — the pay-to-extract margin, GATE-3). ([[systems#Gates|Gates]], [[world#World types|World Types]])
- **#11 Mask/field specifics** — gradient vs. hard radius (leaning gradient); field radius Coalition-only vs. capped player upgrade; confirm asymmetric mask-at-zero; hub vs. Safe Core sustain rates (foreign gates sustain partially — does the Safe Core sustain fully as the newcomer anchor?). ([[systems#Mask energy|Mask Energy]])
...
- **#13 Return-home when the home was wiped mid-run** — if your home was destroyed while you were out, where do the exit dial (your coordinate now points at rubble/hostile soil) and recall land you? Likely a starter/operator anchor — undefined. ([[systems#Gates|Gates]], [[systems#Presence, respawn & XP|Recall]])
...
- **Transponder ledgers over patterns** (USE-1): no-caller-ID is canon (GATE-4 — identity reads at crossing, not at dialing); what remains here is the human layer — accepted-pattern lists for friendly trade, spoofing via sensor-damping tech as the infiltration play.
