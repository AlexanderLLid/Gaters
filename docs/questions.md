...
- **#6 Theme/genre fork — commit the whole game to precursor-tech hybrid?** The leaning is adopted in the combat spine (COMBAT-1: bow/melee → scavenged Builder tech → fleets), the tamer fork resolved narrow (TAME-1), and the visual identity is settled in `INSPIRATION.md`; committing the **whole** game to one setting/tone register is broader than those calls and not yet signed off. Rejected alternatives: pure sci-fi (leaves the starting register open) and pure fantasy reskin (discards the [[gate-physics|Gate Physics]] depth and the space-scale endgame). Scope guards if taken: "easy to use" means _traverse_, not _master_ (dialing stays hard, maps onto the Standard→Heavy→Supergate ladder); fleet combat sits at the _top_ of the climb. Couples to #4 and #19.
...
- **#26 Generator versioning for live worlds.** Changing the worldgen changes what every seed produces — live frontier worlds must not silently mutate, and recorded diffs only make sense against the generator version that created them (already bitten twice in the prototype: stale diffs mis-replayed against a changed layout). Parked until live-ops is real; the world row likely carries a `GenVersion`, with old versions kept generating old worlds or worlds migrated on an amnesty. The cheap half is done in the prototype: `GatersGenVersion` is stamped into every diff save and mismatched diffs are discarded on load, not mis-applied. ([[systems#Gates|FRONTIER-1]])
- **#27 Permanent Gate or temporary Rift?** [open] We are investigating replacing the permanent world Gate with a temporary Rift (possibly projected by a small anchor). Until decided, map generation and streaming must not require Gate-centered topology or a permanent Gate-to-base terrain corridor. This directly reopens the current Gate-only traversal and anti-enclosure calls, so no canon changes until the primary verb and raid seam are re-evaluated together. ([[systems#Gates|Gates]], [[gate-physics]])
