...
- **#25 First-session listing variants** — parked onboarding options, all shaped as broker listing rows on [[systems#First session (START-1)|START-1]]; none needs a new system. **Fresh-spawn rush** — batched fresh charters dial one multi-gate world (FRONTIER-3), each through their own Gate; first imprint converts the world and ends the round; gate entry by charter age (not Potential — a never-building brawler legitimately stays at floor Potential), and the same recipe continues afterward as the permanent floor-tier PvP ladder (BRAWLER-2). **Surge-lane rides** — Safe Core misdials (FRONTIER-2) brokered as free, unread, collapse-countdown dial-outs. **Cert lanes** — pinned `fast_collapse`, trivial loot, once per house; teaches death-is-survivable before ownership. **Recruit rosters** — zero-Potential probation on a host house; recruits return by mask sync, never a dialable coordinate (candidate answer to the member-churn leak). **Mask calibration** — HUD overlays unlock on first-occurrence mind-XP discovery flags; travels with the player, composes with any start. Revisit post-greybox / with population.
