...
This is the **parent repo** for the game Gaters. It holds **design docs only**; the game
project is added later as a **sibling** folder, not nested under `docs/` — chosen over a
code+docs monorepo so the docs mature first and the game drops in clean. Until it exists,
code references stay placeholders. `docs/` is an Obsidian vault.

**Greybox is active** — the standing discipline for all doc and design work
(`.claude/skills/greybox/SKILL.md`): the minimum canon that answers the design question.
